// ====== 1. استدعاء المكتبات المطلوبة ======const admin = require('firebase-admin');const express = require('express');const TelegramBot = require('node-telegram-bot-api');const cors = require('cors'); // مكتبة CORS المضافة// ====== 2. إعداد Firebase ======// سيقوم الخادم بقراءة متغيرات البيئة تلقائياًconst serviceAccount = {type: "service_account",project_id: process.env.FIREBASE_PROJECT_ID,private_key_id: process.env.FIREBASE_PRIVATE_KEY_ID,// يجب معالجة مفتاح Private Key لضمان قراءته بشكل صحيحprivate_key: process.env.FIREBASE_PRIVATE_KEY ? process.env.FIREBASE_PRIVATE_KEY.replace(/\n/g, '\n') : undefined,client_email: process.env.FIREBASE_CLIENT_EMAIL,client_id: process.env.FIREBASE_CLIENT_ID,auth_uri: "https://accounts.google.com/o/oauth2/auth",token_uri: "https://oauth2.googleapis.com/token",auth_provider_x509_cert_url: process.env.FIREBASE_CLIENT_CERT_URL,client_x509_cert_url: process.env.FIREBASE_CLIENT_CERT_URL,universe_domain: "googleapis.com"};try {admin.initializeApp({credential: admin.credential.cert(serviceAccount)});console.log("Firebase connected ✅");} catch (error) {console.error("FIREBASE ERROR:", error.message);process.exit(1); // إنهاء الخادم إذا فشل اتصال Firebase}const db = admin.firestore();// ====== 3. إعداد Express و تشغيل الخادم ======const app = express();const PORT = process.env.PORT || 10000;const token = process.env.TELEGRAM_BOT_TOKEN;const webhookUrl = https://pharmacybotservice.onrender.com/webhook;// ************ FIX: تفعيل مكتبة CORS لحل مشكلة الاتصال ************app.use(cors());// Middleware لمعالجة JSONapp.use(express.json());// FIX: هذا يسمح لـ Express بتقديم الملفات الثابتة من مجلد publicapp.use(express.static('public'));// رسالة ترحيب بسيطة (المسار الأساسي)app.get('/', (req, res) => {// توجيه المستخدم إلى ملف index.html في مجلد publicres.sendFile('index.html', { root: 'public' });});// ====== 4. إعداد Telegram Bot و Webhook ======const bot = new TelegramBot(token, { polling: false });if (!token) {console.error("TELEGRAM_BOT_TOKEN is missing. Cannot start bot.");process.exit(1);} else {console.log("Telegram Token found.");}// إعداد Webhook عند بدء تشغيل الخادمbot.setWebHook(webhookUrl).then(() => {console.log(Webhook set to ${webhookUrl});}).catch(e => {console.error("Error setting webhook:", e.message);});// استقبال الرسائل من تلغرامapp.post(/webhook, (req, res) => {console.log("استلمت طلباً جديداً من تلغرام على المسار /webhook!");bot.processUpdate(req.body);res.sendStatus(200);});// ====== 5. وظيفة البحث في الصيدليات وإرسال الرسائل ======// ************ FIX 1: تم إضافة كلمة 'async' هنا ************app.post('/search-medicine', async (req, res) => {const { medicineName, area } = req.body;// إنشاء معرف فريد للبحث وتخزين الطلب في Firebase
...
});// ====== 6. الاستماع لردود الصيدليات وتحديث Firebase ======// ************ FIX 2: تم إضافة كلمة 'async' هنا ************bot.on('message', async (msg) => {const chatId = msg.chat.id;const text = msg.text;// معالجة الأمر /test للتأكد من أن البوت يعمل
